...
#         logger.info(f"   [Info] Detected Device Mac: {detected_mac}")

#         return detected_mac,formatted_list
...
class SensorProcessor:
    ...
    def fetch_telemetry(self, keyword, start_ts, end_ts):
        ...
        stats = {"total_rows": 0, "valid": 0, "errors": 0}
        ...
        with open(self.data_file, 'r') as f:
            ...
            for row in reader:
                ...
                # CSV Columns: [0] mac_id, [1] sensor_type, [2] timestamp, [3] value
                row_mac  = row[0].strip()
                row_type = row[1].strip() 
                row_time = row[2].strip()  
                row_val  = row[3].strip() 
        ...
        # Return BOTH the MAC ID and the Data List
        return detected_mac, formatted_list
    ...
    def process_sensors_visualize(self, keyword, start_date, end_date, title):
        ...
        # Unpack MAC ID and Data List
        mac_id, telemetry_data = self.fetch_telemetry(keyword, start_ts, end_ts)
        
        if not telemetry_data:
            logger.warning("No data found, skipping visualization.")
            return

        # Create new title with MAC ID
        final_title = f"{title} - MAC: {mac_id}"
        
        # Pass the DATA LIST (not the tuple) to visualization
        self.visualize_telemetry(final_title, telemetry_data)
        logger.info("="*60)
